...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, ch_in=64, ch_out=128, in_place=False):
    ...
    def forward(self, x):
        res = self.res(x)
        # x = self.identity(x)
        return torch.add(self.identity(x), res)
...
class ResidualInceptionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.identity(x)
        return torch.add(self.identity(x), self.filter_concat(res))
...
class BottomUpBlock(nn.Module):
    ...
    def forward(self, x, res):
        # x = self.upsample(x)
        return self.res_block(self.upsample(x) + res)
...
class HourGlassNetMultiScaleInt(nn.Module):
    ...
    def forward(self, x0, features=None):
        x = self.conv_in(x0)
        SR_map = []
        result = []
        out = x
        ...
        read_feature = False
        if features is not None: read_feature = True
        ...
        for i in range(self.n_HG):
            ...
            if i in super_block_idx:
                ...
                if super_block_idx.index(i) == 0:
                    ...
                    if read_feature: features.append(sr_feature)
    ...
    def forward_part1(self, x):
        ...
        for i in range(self.n_HG):
            ...
            if i in super_block_idx:
                ...
                if super_block_idx.index(i) in [0, 1, 2]:
                    features.append(sr_feature)

        return features

    def forward_part2(self, features, SR_map_help):
        SR_map = []
        result = []
        ...
        for i in range(self.n_HG):
            if i in super_block_idx:
                if super_block_idx.index(i) == 0:
                    sr_feature = features[feature_idx]
                    feature_idx += 1
                    srout_flat = self.upsample_flat(sr_feature)
                    flat_map = self.flat_map(sr_feature)
                    result.append(srout_flat)
    ...
    def forward_with_map(self, x, SR_map_help):
        flat_map, edge_map, corner_map = SR_map_help
...
class HourGlassNetMultiScaleInt_Origin(nn.Module):
    """
    Hour Glass SR Model, Use Mutil-Scale Label(HR_down_Xn) Supervision.
    """
    ...
    def __init__(self, in_nc=3, out_nc=3, upscale=4, nf=64, res_type='res', n_mid=2, n_tail=2, n_HG=6,
                 act_type='leakyrelu', inter_supervis=True, mscale_inter_super=False, share_upsample=False):
        super(HourGlassNetMultiScaleInt_Origin, self).__init__()
